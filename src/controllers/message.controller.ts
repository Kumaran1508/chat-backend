...
import { AcknowledgeResponse, DeliveredRequest, ReadRequest, Message, MessageRequest, MessageSchema, MessageResponse, MessageStatus } from "../interfaces/message.interface";
...
export default class MessageController {
    ...
    public init(){
        ...
        this.webSocketServer.on('connection', async socket => {
            ...
            const token = socket.request.headers.authorization;
            const username = await (decode(token.split(" ")[1]) as JwtPayload)['username'];
    ...
    private addOnDeliveredListener(socket: Socket) {
        socket.on(SocketEvent.DELIVERED, async data =>{
            try {
                ...
                const requester = this.messageService.getUser(deliveredRequest.acknowledgedBy);
                ...
                const message = this.messageService.getMessage(deliveredRequest.messageId);
    ...
    addOnReadListener(socket: Socket) {
        socket.on(SocketEvent.READ, async data =>{
            try {
                ...
                const requester = this.messageService.getUser(readRequest.acknowledgedBy);
